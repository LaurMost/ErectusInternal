...
bool PlayerStatsEditor::Edit(const bool enabled)
{
    ...
    if (enabled)
    {
        ...
        {
            // Page is RX - check if update needed
            ...
            {
                // Use ReprotectWrite for RX page updates
                if (!ErectusProcess::ReprotectWrite(actorValuePage, &actorValueHookData, sizeof actorValueHookData))
                    return false;
            }
        }
        else
        {
            // First time setup - write and protect
            if (!ErectusProcess::Wpm(actorValuePage, &actorValueHookData, sizeof actorValueHookData))
                return false;
            if (!ErectusProcess::ProtectEx(actorValuePage, sizeof(ActorValueHook)))
                return false;
