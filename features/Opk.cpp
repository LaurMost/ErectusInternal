...
bool Opk::OnePositionKill(const bool enabled)
{
    ...
    if (!opkPage)
    {
        const auto page = ErectusProcess::AllocEx(sizeof(OpkData));
        if (!page)
            return false;
        opkPage = page;
    }
    ...
    if (enabled)
    {
        ...
        // Protect to RX
        if (!ErectusProcess::ProtectEx(opkPage, sizeof(OpkData)))
            return false;
...
bool Opk::SetOpkData(const bool enabled)
{
    ...
    if (!enabled)
    {
        ...
        return ErectusProcess::ReprotectWrite(opkPage, &opkData, sizeof opkData);
    }
